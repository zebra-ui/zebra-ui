import {
  d as l,
  k as e,
  m as i,
  G as a,
  o as t,
  c as r,
  w as o,
  a as c,
  e as f,
  r as s,
  E as n,
  F as d,
  H as u,
  i as p,
  _ as m
} from './index-7fd204f3.js'
import { r as b, _ as g, a as h } from './DemoPage.0bae30a9.js'
import { _ as k, a as w } from './z-tabs.8d3e6868.js'
import './useParent.5b9c05b5.js'
import './use-id.c1b053ed.js'
import './z-sticky.e6c39ae3.js'
import './useChildren.fb2421ae.js'
import './use-touch.1ac9fd53.js'
const y = {
    id: '4468162',
    name: 'zebra',
    font_family: 'z-icon',
    css_prefix_text: 'z-icon-',
    description: 'zebra-ui组件库',
    basic: [
      'check',
      'close',
      'minus',
      'plus',
      'right',
      'left',
      'up',
      'down',
      'right-big',
      'left-big',
      'up-big',
      'down-big',
      'arrowright',
      'arrowleft',
      'arrowup',
      'arrowdown',
      'right-circle-fill',
      'left-circle-fill',
      'up-circle-fill',
      'down-circle-fill',
      'plus-circle-fill',
      'minus-circle-fill',
      'check-circle-fill',
      'close-circle-fill',
      'check-circle',
      'close-circle',
      'plus-circle',
      'minus-circle',
      'right-circle',
      'left-circle',
      'up-circle',
      'down-circle'
    ],
    outline: [
      'moon',
      'sun',
      'delete-key',
      'hide-key',
      'miniprogram',
      'funnelplot',
      'like',
      'unlike',
      'unlock',
      'lock',
      'customerservice',
      'flag',
      'moneycollect',
      'medicinebox',
      'shop',
      'rocket',
      'shopping',
      'folder',
      'folder-open',
      'folder-add',
      'deploymentunit',
      'accountbook',
      'contacts',
      'carryout',
      'calendar-check',
      'calendar',
      'scan',
      'select',
      'boxplot',
      'build',
      'sliders',
      'laptop',
      'barcode',
      'camera',
      'cluster',
      'gateway',
      'car',
      'printer',
      'read',
      'cloud-server',
      'cloud-upload',
      'cloud',
      'cloud-download',
      'alibaba',
      'cloud-sync',
      'antdesign',
      'video',
      'notification',
      'behance',
      'sound',
      'googleplus',
      'radarchart',
      'medium',
      'qrcode',
      'google',
      'fund',
      'IE',
      'image',
      'amazon',
      'mail',
      'slack',
      'table',
      'alipay',
      'idcard',
      'taobao',
      'creditcard',
      'zhihu',
      'heart',
      'HTML',
      'block',
      'linkedin',
      'error',
      'yahoo',
      'star',
      'facebook',
      'gold',
      'skype',
      'heatmap',
      'CodeSandbox',
      'wifi',
      'chrome',
      'attachment',
      'codepen',
      'edit',
      'aliwangwang',
      'key',
      'apple',
      'api',
      'android',
      'disconnect',
      'sketch',
      'highlight',
      'Gitlab',
      'monitor',
      'dribbble',
      'link',
      'instagram',
      'man',
      'reddit',
      'percentage',
      'windows',
      'pushpin',
      'yuque',
      'phone',
      'Youtube',
      'shake',
      'tag',
      'wrench',
      'tags',
      'scissor',
      'mr',
      'share',
      'branches',
      'fork',
      'shrink',
      'arrawsalt',
      'weibo',
      'verticalright',
      'verticalleft',
      'fullscreen',
      'fullscreen-exit',
      'doubleleft',
      'doubleright',
      'upload',
      'colum-height',
      'vertical-align-botto',
      'vertical-align-middl',
      'totop',
      'vertical-align-top',
      'download',
      'sort-descending',
      'sort-ascending',
      'fall',
      'swap',
      'stock',
      'rise',
      'indent',
      'outdent',
      'menu',
      'unorderedlist',
      'orderedlist',
      'align-right',
      'align-center',
      'align-left',
      'pic-center',
      'pic-right',
      'pic-left',
      'bold',
      'font-colors',
      'exclaimination',
      'font-size',
      'CI',
      'infomation',
      'Dollar',
      'line-height',
      'compass',
      'strikethrough',
      'underline',
      'frown',
      'number',
      'info-circle',
      'italic',
      'code',
      'column-width',
      'EURO',
      'zoomout',
      'copyright',
      'ellipsis',
      'apartment',
      'dash',
      'audio',
      'meh',
      'enter',
      'robot',
      'play-circle',
      'line',
      'zoomin',
      'question-circle',
      'Pound',
      'question',
      'rollback',
      'bug',
      'smile',
      'small-dash',
      'audiostatic',
      'trademark',
      'pause',
      'comment',
      'time-circle',
      'bg-colors',
      'timeout',
      'crown',
      'verified',
      'earth',
      'drag',
      'YUAN',
      'desktop',
      'videocameraadd',
      'gift',
      'switchuser',
      'warning-circle',
      'stop',
      'whatsapp',
      'sync',
      'fire',
      'appstoreadd',
      'transaction',
      'thunderbolt',
      'caret-down',
      'undo',
      'backward',
      'redo',
      'caret-up',
      'reload',
      'caret-right',
      'reloadtime',
      'caret-left',
      'message',
      'fast-backward',
      'dashboard',
      'forward',
      'issuesclose',
      'fast-forward',
      'poweroff',
      'search',
      'logout',
      'retweet',
      'piechart',
      'login',
      'setting',
      'step-backward',
      'eye',
      'step-forward',
      'location',
      'swap-right',
      'edit-square',
      'swap-left',
      'export',
      'woman',
      'save',
      'Import',
      'appstore',
      'eye-close',
      'close-square',
      'clear',
      'down-square',
      'collapse',
      'layout',
      'expand',
      'left-square',
      'deletecolumn',
      'play-square',
      'merge-cells',
      'control',
      'subnode',
      'codelibrary',
      'rotate-left',
      'detail',
      'rotate-right',
      'minus-square',
      'insertrowbelow',
      'plus-square',
      'insertrowabove',
      'right-square',
      'table1',
      'project',
      'solit-cells',
      'wallet',
      'formatpainter',
      'up-square',
      'insertrowright',
      'calculator',
      'interation',
      'insertrowleft',
      'check-square',
      'translate',
      'border',
      'deleterow',
      'border-outer',
      'sisternode',
      'border-top',
      'Field-number',
      'border-bottom',
      'Field-String',
      'border-left',
      'Function',
      'border-right',
      'Field-time',
      'border-inner',
      'GIF',
      'border-verticle',
      'Partition',
      'border-horizontal',
      'index',
      'radius-bottomleft',
      'Storedprocedure',
      'radius-bottomright',
      'Field-Binary',
      'radius-upleft',
      'Console-SQL',
      'radius-upright',
      'icon-test',
      'radius-setting',
      'aim',
      'adduser',
      'compress',
      'deleteteam',
      'expend',
      'deleteuser',
      'folder-view',
      'addteam',
      'file-GIF',
      'user',
      'group',
      'team',
      'send',
      'Report',
      'linechart',
      'View',
      'barchart',
      'shortcut',
      'pointmap',
      'ungroup',
      'container',
      'database',
      'sever',
      'mobile',
      'tablet',
      'redenvelope',
      'book',
      'filedone',
      'reconciliation',
      'file-exception',
      'filesync',
      'filesearch',
      'solution',
      'fileprotect',
      'file-add',
      'file-excel',
      'file-exclamation',
      'file-pdf',
      'file-image',
      'file-markdown',
      'file-unknown',
      'file-ppt',
      'file-word',
      'file',
      'file-zip',
      'file-text',
      'file-copy',
      'snippets',
      'audit',
      'diff',
      'Batchfolding',
      'securityscan',
      'propertysafety',
      'safetycertificate',
      'insurance',
      'alert',
      'delete',
      'hourglass',
      'bulb',
      'experiment',
      'bell',
      'trophy',
      'rest',
      'USB',
      'skin',
      'home',
      'bank',
      'filter'
    ],
    filled: [
      'miniprogram-fill',
      'home-fill',
      'trophy-fill',
      'location-fill',
      'cloud-fill',
      'customerservice-fill',
      'experiment-fill',
      'eye-fill',
      'like-fill',
      'lock-fill',
      'unlike-fill',
      'star-fill',
      'unlock-fill',
      'alert-fill',
      'api-fill',
      'highlight-fill',
      'phone-fill',
      'edit-fill',
      'pushpin-fill',
      'rocket-fill',
      'thunderbolt-fill',
      'tag-fill',
      'wrench-fill',
      'tags-fill',
      'bank-fill',
      'camera-fill',
      'error-fill',
      'crown-fill',
      'mail-fill',
      'car-fill',
      'printer-fill',
      'shop-fill',
      'setting-fill',
      'USB-fill',
      'golden-fill',
      'build-fill',
      'boxplot-fill',
      'sliders-fill',
      'Gitlab-fill',
      'android-fill',
      'apple-fill',
      'HTML-fill',
      'windows-fill',
      'skype-fill',
      'yuque-fill',
      'Youtube-fill',
      'yahoo-fill',
      'wechat-fill',
      'chrome-fill',
      'alipay-circle-fill',
      'aliwangwang-fill',
      'behance-circle-fill',
      'amazon-circle-fill',
      'codepen-circle-fill',
      'dropbox-circle-fill',
      'github-fill',
      'dribbble-circle-fill',
      'medium-circle-fill',
      'QQ-circle-fill',
      'IE-circle-fill',
      'google-circle-fill',
      'dingtalk-circle-fill',
      'sketch-circle-fill',
      'slack-circle-fill',
      'twitter-circle-fill',
      'taobao-circle-fill',
      'weibo-circle-fill',
      'zhihu-circle-fill',
      'reddit-circle-fill',
      'alipay-square-fill',
      'dingtalk-square-fill',
      'behance-square-fill',
      'amazon-square-fill',
      'codepen-square-fill',
      'dribbble-square-fill',
      'dropbox-square-fill',
      'facebook-fill',
      'google-square-fill',
      'instagram-fill',
      'IE-square-fill',
      'medium-square-fill',
      'linkedin-fill',
      'QQ-square-fill',
      'reddit-square-fill',
      'twitter-square-fill',
      'sketch-square-fill',
      'slack-square-fill',
      'taobao-square-fill',
      'weibo-square-fill',
      'zhihu-square-fill',
      'audio-fill',
      'robot-fill',
      'bug-fill',
      'signal-fill',
      'shortcut-fill',
      'info-circle-fill',
      'question-circle-fill',
      'EURO-circle-fill',
      'frown-fill',
      'CI-circle-fill',
      'compass-fill',
      'Dollar-circle-fill',
      'poweroff-circle-fill',
      'eyeclose-fill',
      'meh-fill',
      'play-circle-fill',
      'Pound-circle-fill',
      'smile-fill',
      'stop-fill',
      'warning-circle-fill',
      'time-circle-fill',
      'YUAN-circle-fill',
      'heart-fill',
      'dashboard-fill',
      'message-fill',
      'check-square-fill',
      'down-square-fill',
      'minus-square-fill',
      'formatpainter-fill',
      'close-square-fill',
      'codelibrary-fill',
      'left-square-fill',
      'play-square-fill',
      'up-square-fill',
      'right-square-fill',
      'plus-square-fill',
      'accountbook-fill',
      'carryout-fill',
      'calendar-fill',
      'calculator-fill',
      'interation-fill',
      'project-fill',
      'detail-fill',
      'save-fill',
      'wallet-fill',
      'control-fill',
      'layout-fill',
      'appstore-fill',
      'mobile-fill',
      'tablet-fill',
      'book-fill',
      'redenvelope-fill',
      'propertysafety-fill',
      'insurance-fill',
      'securityscan-fill',
      'file-add-fill',
      'file-fill',
      'file-excel-fill',
      'file-markdown-fill',
      'file-text-fill',
      'file-ppt-fill',
      'file-unknown-fill',
      'file-word-fill',
      'file-zip-fill',
      'file-pdf-fill',
      'file-image-fill',
      'diff-fill',
      'file-copy-fill',
      'snippets-fill',
      'batchfolding-fill',
      'reconciliation-fill',
      'folder-add-fill',
      'folder-fill',
      'folder-open-fill',
      'database-fill',
      'container-fill',
      'sever-fill',
      'calendar-check-fill',
      'image-fill',
      'idcard-fill',
      'creditcard-fill',
      'fund-fill',
      'read-fill',
      'contacts-fill',
      'delete-fill',
      'notification-fill',
      'flag-fill',
      'moneycollect-fill',
      'medicinebox-fill',
      'rest-fill',
      'shopping-fill',
      'skin-fill',
      'video-fill',
      'sound-fill',
      'bulb-fill',
      'bell-fill',
      'filter-fill',
      'fire-fill',
      'funnelplot-fill',
      'gift-fill',
      'hourglass-fill',
      'ant-cloud',
      'QQ',
      'twitter',
      'dingtalk',
      'dropbox',
      'CodeSandbox-circle-f',
      'googleplus-circle-f',
      'CodeSandbox-square-f',
      'googleplus-square-f',
      'copyright-circle-fil',
      'trademark-circle-fil',
      'piechart-circle-fil',
      'safetycertificate-f',
      'file-exclamation-fil',
      'areachart'
    ]
  },
  q = m(
    l({
      __name: 'icon',
      setup(l) {
        const m = e(0),
          q = 'smile',
          _ = 'https://cdn.zebraui.com/zebra-ui/images/assets/demo-select.png',
          x = (l) => {
            u({ data: l })
          }
        return (l, e) => {
          const u = b(i('z-icon'), g),
            z = p,
            v = a('demo-block'),
            C = b(i('z-tab'), k),
            j = b(i('z-tabs'), w),
            I = b(i('DemoPage'), h)
          return (
            t(),
            r(
              I,
              { title: 'Icon' },
              {
                default: o(() => [
                  c(
                    z,
                    { class: 'demo-icon' },
                    {
                      default: o(() => [
                        c(
                          j,
                          {
                            active: m.value,
                            'onUpdate:active':
                              e[9] || (e[9] = (l) => (m.value = l)),
                            sticky: '',
                            'custom-navbar': ''
                          },
                          {
                            default: o(() => [
                              c(
                                C,
                                { title: '用法示例' },
                                {
                                  default: o(() => [
                                    c(
                                      v,
                                      { title: '基础用法' },
                                      {
                                        default: o(() => [
                                          c(
                                            z,
                                            { class: 'list' },
                                            {
                                              default: o(() => [
                                                c(
                                                  z,
                                                  { class: 'item' },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: q,
                                                        size: '68rpx',
                                                        onClick:
                                                          e[0] ||
                                                          (e[0] = (l) => x(q))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                ),
                                                c(
                                                  z,
                                                  { class: 'item' },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: _,
                                                        size: '68rpx',
                                                        onClick:
                                                          e[1] ||
                                                          (e[1] = (l) => x(_))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                )
                                              ]),
                                              _: 1
                                            }
                                          )
                                        ]),
                                        _: 1
                                      }
                                    ),
                                    c(
                                      v,
                                      { title: '提示信息' },
                                      {
                                        default: o(() => [
                                          c(
                                            z,
                                            { class: 'list' },
                                            {
                                              default: o(() => [
                                                c(
                                                  z,
                                                  { class: 'item' },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: q,
                                                        size: '64rpx',
                                                        dot: '',
                                                        onClick:
                                                          e[2] ||
                                                          (e[2] = (l) => x(q))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                ),
                                                c(
                                                  z,
                                                  { class: 'item' },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: q,
                                                        size: '64rpx',
                                                        badge: '9',
                                                        onClick:
                                                          e[3] ||
                                                          (e[3] = (l) => x(q))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                ),
                                                c(
                                                  z,
                                                  { class: 'item' },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: q,
                                                        size: '64rpx',
                                                        badge: '99+',
                                                        onClick:
                                                          e[4] ||
                                                          (e[4] = (l) => x(q))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                )
                                              ]),
                                              _: 1
                                            }
                                          )
                                        ]),
                                        _: 1
                                      }
                                    ),
                                    c(
                                      v,
                                      { title: '图标颜色' },
                                      {
                                        default: o(() => [
                                          c(
                                            z,
                                            { class: 'list' },
                                            {
                                              default: o(() => [
                                                c(
                                                  z,
                                                  { class: 'item' },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: q,
                                                        size: '64rpx',
                                                        color: '#1989fa',
                                                        onClick:
                                                          e[5] ||
                                                          (e[5] = (l) => x(q))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                ),
                                                c(
                                                  z,
                                                  { class: 'item' },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: q,
                                                        size: '64rpx',
                                                        color: '#07c160',
                                                        onClick:
                                                          e[6] ||
                                                          (e[6] = (l) => x(q))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                )
                                              ]),
                                              _: 1
                                            }
                                          )
                                        ]),
                                        _: 1
                                      }
                                    ),
                                    c(
                                      v,
                                      { title: '图标大小' },
                                      {
                                        default: o(() => [
                                          c(
                                            z,
                                            { class: 'list' },
                                            {
                                              default: o(() => [
                                                c(
                                                  z,
                                                  { class: 'item' },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: q,
                                                        size: '50',
                                                        onClick:
                                                          e[7] ||
                                                          (e[7] = (l) => x(q))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                ),
                                                c(
                                                  z,
                                                  {
                                                    class: 'item',
                                                    style: {
                                                      'margin-left': '20rpx'
                                                    }
                                                  },
                                                  {
                                                    default: o(() => [
                                                      c(u, {
                                                        name: q,
                                                        size: '80',
                                                        onClick:
                                                          e[8] ||
                                                          (e[8] = (l) => x(q))
                                                      })
                                                    ]),
                                                    _: 1
                                                  }
                                                )
                                              ]),
                                              _: 1
                                            }
                                          )
                                        ]),
                                        _: 1
                                      }
                                    )
                                  ]),
                                  _: 1
                                }
                              ),
                              c(
                                C,
                                { title: '基础图标' },
                                {
                                  default: o(() => [
                                    c(
                                      v,
                                      { title: 'basic' },
                                      {
                                        default: o(() => [
                                          c(
                                            z,
                                            { class: 'list' },
                                            {
                                              default: o(() => [
                                                (t(!0),
                                                f(
                                                  d,
                                                  null,
                                                  s(
                                                    n(y).basic,
                                                    (l, e) => (
                                                      t(),
                                                      r(
                                                        z,
                                                        {
                                                          class: 'item',
                                                          key: e
                                                        },
                                                        {
                                                          default: o(() => [
                                                            c(
                                                              u,
                                                              {
                                                                name: l,
                                                                size: '64rpx',
                                                                'custom-class':
                                                                  'icon',
                                                                onClick: (e) =>
                                                                  x(l)
                                                              },
                                                              null,
                                                              8,
                                                              [
                                                                'name',
                                                                'onClick'
                                                              ]
                                                            )
                                                          ]),
                                                          _: 2
                                                        },
                                                        1024
                                                      )
                                                    )
                                                  ),
                                                  128
                                                ))
                                              ]),
                                              _: 1
                                            }
                                          )
                                        ]),
                                        _: 1
                                      }
                                    )
                                  ]),
                                  _: 1
                                }
                              ),
                              c(
                                C,
                                { title: '线性图标' },
                                {
                                  default: o(() => [
                                    c(
                                      v,
                                      { title: 'line' },
                                      {
                                        default: o(() => [
                                          c(
                                            z,
                                            { class: 'list' },
                                            {
                                              default: o(() => [
                                                (t(!0),
                                                f(
                                                  d,
                                                  null,
                                                  s(
                                                    n(y).outline,
                                                    (l, e) => (
                                                      t(),
                                                      r(
                                                        z,
                                                        {
                                                          class: 'item',
                                                          key: e
                                                        },
                                                        {
                                                          default: o(() => [
                                                            c(
                                                              u,
                                                              {
                                                                name: l,
                                                                size: '64rpx',
                                                                'custom-class':
                                                                  'icon',
                                                                onClick: (e) =>
                                                                  x(l)
                                                              },
                                                              null,
                                                              8,
                                                              [
                                                                'name',
                                                                'onClick'
                                                              ]
                                                            )
                                                          ]),
                                                          _: 2
                                                        },
                                                        1024
                                                      )
                                                    )
                                                  ),
                                                  128
                                                ))
                                              ]),
                                              _: 1
                                            }
                                          )
                                        ]),
                                        _: 1
                                      }
                                    )
                                  ]),
                                  _: 1
                                }
                              ),
                              c(
                                C,
                                { title: '面性图标' },
                                {
                                  default: o(() => [
                                    c(
                                      v,
                                      { title: 'fill' },
                                      {
                                        default: o(() => [
                                          c(
                                            z,
                                            { class: 'list' },
                                            {
                                              default: o(() => [
                                                (t(!0),
                                                f(
                                                  d,
                                                  null,
                                                  s(
                                                    n(y).filled,
                                                    (l, e) => (
                                                      t(),
                                                      r(
                                                        z,
                                                        {
                                                          class: 'item',
                                                          key: e
                                                        },
                                                        {
                                                          default: o(() => [
                                                            c(
                                                              u,
                                                              {
                                                                name: l,
                                                                size: '64rpx',
                                                                'custom-class':
                                                                  'icon',
                                                                onClick: (e) =>
                                                                  x(l)
                                                              },
                                                              null,
                                                              8,
                                                              [
                                                                'name',
                                                                'onClick'
                                                              ]
                                                            )
                                                          ]),
                                                          _: 2
                                                        },
                                                        1024
                                                      )
                                                    )
                                                  ),
                                                  128
                                                ))
                                              ]),
                                              _: 1
                                            }
                                          )
                                        ]),
                                        _: 1
                                      }
                                    )
                                  ]),
                                  _: 1
                                }
                              )
                            ]),
                            _: 1
                          },
                          8,
                          ['active']
                        )
                      ]),
                      _: 1
                    }
                  )
                ]),
                _: 1
              }
            )
          )
        }
      }
    }),
    [['__scopeId', 'data-v-8fb7d9ee']]
  )
export { q as default }
