export default function Navigation({ swiper, extendParams, on, emit }) {
  ...
  function toggleEl($el, disabled) {
    ...
    if ($el) {
      swiper.$wrapperEl[disabled ? `add${$el}` : `remove${$el}`](
        params.disabledClass
      )
      if (swiper.params.watchOverflow && swiper.enabled) {
        swiper.$wrapperEl[swiper.isLocked ? `add${$el}` : `remove${$el}`](
          params.lockClass
        )
      }
    }
  ...
  on('enable disable', () => {
    ...
    if ($nextEl) {
      swiper.$wrapperEl[
        swiper.enabled ? 'removeNextElClass' : 'addNextElClass'
      ](swiper.params.navigation.lockClass)
      // $nextEl[swiper.enabled ? 'removeClass' : 'addClass'](swiper.params.navigation.lockClass);
    }
    if ($prevEl) {
      swiper.$wrapperEl[
        swiper.enabled ? 'removePrevElClass' : 'addPrevElClass'
      ](swiper.params.navigation.lockClass)
