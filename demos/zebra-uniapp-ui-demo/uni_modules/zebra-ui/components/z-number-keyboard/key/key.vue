<template>
	<view :class="[bemMethods('key__wrapper',{'wider': wider})]" @click="onClick">
		<view :class="[bemMethods('key__content',[color, { large, active, delete:type==='delete'}])]"
			:hover-class="bemMethods('key__content--active',[color])" hover-stay-time="70">
			<template v-if="loading || type === 'delete' || type === 'extra'">
				<view v-if="loading" class="zebra-key__content__loading-icon">
					<z-loading></z-loading>
				</view>
				<view v-if="type === 'delete'" class="zebra-key__content__delete-icon">
					<template v-if="text">
						{{text}}
					</template>
					<template v-else>
				...
				<view v-if="type === 'extra'" class="zebra-key__content__collapse-icon">
					<template v-if="text">
						{{text}}
					</template>
					<template v-else>
